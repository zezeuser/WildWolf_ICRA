...
def train_and_evaluate(args):
    ...
    # buffer = ReplayBuffer(max_len=max_memo + max_step, state_dim=state_dim, action_dim=action_dim,
    #                     if_on_policy=if_on_policy, if_per=if_per, if_gpu=True)
...
def get_episode_return(env, act, device) -> (float, int):
    ...
    for episode_step in range(max_step):
        s_tensor = torch.as_tensor((state),  dtype=torch.float32,device=device)     # 报错 expected scalar type Double but found Float 修改成(state), dtype=torch.float32
        a_tensor = act(s_tensor) 
        if if_discrete:
            a_tensor = a_tensor.argmax(dim=1)
        action = a_tensor.detach().cpu().numpy()[0]  # not need detach(), because with torch.no_grad() outside
        for action_ind in range(0,7):    #action_dim = 7
            if action[action_ind] > 0 :
                if action[action_ind] > 0.5:
                    action[action_ind] = 1
                else: action[action_ind] = 0
            else:
                if action[action_ind] < -0.5 :
                    action[action_ind] = -1
                else: action[action_ind] = 0  
...
def explore_before_training(env, buffer, target_step, reward_scale, gamma) -> int:
    ...
    while steps < target_step:
        # action = rd.randint(-1,1,size=action_dim) if if_discrete else rd.uniform(-1, 1, size=action_dim)
        action = rd.randint(-1,2,size=action_dim) if if_discrete else rd.randint(-1,2,size=action_dim)
        # print(action)
