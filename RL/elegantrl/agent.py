...
class AgentBase:
    ...
    def explore_env(self, env, buffer, target_step, reward_scale, gamma) -> int:
        ...
        for _ in range(target_step):
            action = self.select_action(self.state)[0]
            for action_ind in range(0,7):
...
class AgentDoubleDQN(AgentDQN):
    ...
    def get_obj_critic_raw(self, buffer, batch_size):
        with torch.no_grad():
            reward, mask, action, state, next_s = buffer.sample_batch(batch_size)
            next_q = torch.min(*self.cri_target.get_q1_q2(next_s))   #next_q.shape = [256 , 7]
            next_q = next_q.max(dim=1, keepdim=True)[0]    #next_q.shape = [256,1]
            q_label = reward + mask * next_q
        act_int = action.type(torch.long)    #act_int.shape = [256,7]
        q1, q2 = [qs.gather(1, act_int) for qs in self.act.get_q1_q2(state)]   #q1.shape = q2.shape = [256,7]   qs.shape = [256,7]
