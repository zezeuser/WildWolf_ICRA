...
namespace roborts_decision{
  ...
  public:
    ...
    void TurnToDetectedDirection() {      
      if(blackboard_ptr_->IfTurn()){
        ...
        try{ 
          ...
          hurt_pose.header.frame_id = "red1/base_link";
    ...
    void CostMapFiltar(geometry_msgs::PoseStamped pose1){
      ...
      filtar_pose = pose1;
        ...
        if (blackboard_ptr_->GetCostMap2D()->GetCost(goal_cell_x, goal_cell_y) >= 253) {   
          ...
          for(base_local_planner::LineIterator  line( goal_cell_x, goal_cell_y, robot_cell_x, robot_cell_y); line.isValid(); line.advance()) {     
    ...
    void ScanViewInit(){
        ...
        gimbal_tf_pose.frame_id_ = "red1/shoot_link";  
        ...
        try{
            ...
            blackboard_ptr_->GetTFptr()->transformPose("red1/base_link", gimbal_pose, gimbal_base_pose);
        }
    ...
    void Speech_go(){
       ROS_INFO("start speech goal");
       int  speech_count=blackboard_ptr_->GetSpeechcount();
       if (speech_count >=7)
       {
            switch (speech_count)
            {
          ...
          case 1:
               /* code */
               pose = blackboard_ptr_ ->GetSearchPose(0);
               chassis_executor_ptr_ ->Execute(pose);
               break;
          case 2:
               pose = blackboard_ptr_ ->GetSearchPose(1);
               chassis_executor_ptr_ ->Execute(pose);

          case 3:
               pose = blackboard_ptr_ ->GetSearchPose(2);
               chassis_executor_ptr_ ->Execute(pose);

          case 4:
               pose = blackboard_ptr_ ->GetSearchPose(3);
               chassis_executor_ptr_ ->Execute(pose);

          case 5:
               pose = blackboard_ptr_ ->GetSearchPose(4);
               chassis_executor_ptr_ ->Execute(pose);    

          case 6:
               pose = blackboard_ptr_ ->GetSearchPose(5);
               chassis_executor_ptr_ ->Execute(pose);   
