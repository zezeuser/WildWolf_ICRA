...
#include "action_node/speech_area.h"
...
int main(int argc, char **argv){    
   ...
   ROS_INFO("choose action");
    ...
    auto speech_action = std::make_shared<roborts_decision::Speech_area>(black_ptr, g_factory);               //       
    //----------------------------------------------------------------------------------------------------------------------------------
    ...
    while(ros::ok()){
        ...
        switch (command) {
            ...
            case '11':
                ...
                // game_status_selector->AddChildren(froze_action);
                // root.Run();
                game_status_selector->AddChildren(speech_action);
                game_status_selector->AddChildren(froze_action);
                root.Run();
                break;
            case 27:
