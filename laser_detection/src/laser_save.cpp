...
//  }
  try{
    listener_.lookupTransform("/odom", "/laser_link", ros::Time(0), transform_laser);
  }catch (tf::TransformException ex){
    return;
  }
  std::printf("it is ok\n");
  sensor_msgs::PointCloud cloud;
  projector_.transformLaserScanToPointCloud("/laser_link",*scan_in,cloud,listener_);
  ...
  *laser_cloud = cloud_out;
  ...
  char laser_f[100];
  std::sprintf(laser_f, "/home/zeze/catkin_ws/src/laser_detection/map_cloud/laser_%d.pcd", laser_count);
